...
import { updateInvoice, markPaid } from "@/lib/invoices";
...
export async function PATCH(
  ...
  try {
    ...
    const status: Status | undefined = body?.status as Status | undefined;
    ...
    if (status === "PAID" && payment_method) {
      const done = await markPaid(id, payment_method);
      if (!done) {
