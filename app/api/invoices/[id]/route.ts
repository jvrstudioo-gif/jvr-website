...
export async function PATCH(
  ...
  const body = await req.json().catch(() => ({} as Partial<Invoice>));


  const allowedStatus = ["DRAFT", "SENT", "PAID", "OVERDUE", "VOID"] as const;
  const allowedMethods = ["square_card", "square_tap", "cash", null] as const;
  ...
  if (body.status) {
    if (!allowedStatus.includes(body.status as any)) {
  ...
  if ("payment_method" in body) {
    if (!allowedMethods.includes(body.payment_method as any)) {
      return NextResponse.json({ error: "Invalid payment_method" }, { status: 400 });
    }
    inv.payment_method = body.payment_method as Invoice["payment_method"];
  }
